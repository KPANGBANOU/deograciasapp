...
class HistoriqueStockCumuleCentre extends StatelessWidget {
  const HistoriqueStockCumuleCentre({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    produits.forEach((element) {
      total_produit += element.montant_vendu_cumule;
      benefice_produit += element.benefice_cumule;
    });

    tee_shirts.forEach((element) {
      total_tee_shirt += element.montant_vendu_cumule;
      benefice_tee_shirt += element.benefice_cumule;
    });

    credits.forEach((element) {
      total_credits +=
          (element.montant_initial_cumule - element.montant_disponible);
      benefice_credit += element.benefice_cumule;
    });

    photocopies.forEach((element) {
      total_photocopie += element.montant_vendu_cumule;
      benefice_photocopie += element.benefice_cumule;
    });
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            Text(
              "Centre informatique stocks cumulés ",
                              ...
                              child: DataTable(
                                  ...
                                  rows: credits.map((produit) {
                                    return DataRow(cells: [
                                      ...
                                      DataCell(Text(
                                        produit.montant_initial_cumule
                                                .toString() +
                                            " XOF",
                                        style: GoogleFonts.alike(
                                            color: Colors.lightBlue.shade800,
